...
export default async function AdminConcoursPage({
  ...
  let dataQuery = db
    .from('races')
    .select(
      'id, race_date, release_point, category, age_class, pigeons_released, distance_min_km, clubs(name), race_pdfs(pdf_url, pdf_title)',
    )
    ...
    <main style={{ padding: '32px 36px 60px' }}>
      <div style={{ marginBottom: 20 }}>
        ...
        <p style={{ margin: '4px 0 0', color: '#64748b', fontSize: 14 }}>
          Courses importées depuis Francolomb
        </p>
      </div>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 14, flexWrap: 'wrap' }}>
        <form
          method="GET"
          style={{ display: 'flex', gap: 0, flex: 1, minWidth: 200, maxWidth: 320 }}
        >
          <input
            name="q"
            defaultValue={q}
            placeholder="Rechercher un lieu de lâcher..."
            style={{
              flex: 1,
              padding: '8px 12px',
              border: '1px solid #e2e8f0',
            ...
            <tbody>
              {(races ?? []).length === 0 ? (
                ...
                (races ?? []).map((r) => {
                  ...
                  const pdfs = r.race_pdfs as unknown as
                    | { pdf_url: string; pdf_title: string | null }[]
                    | null;
                  ...
                  return (
                    <tr key={r.id} className="admin-tr">
                      ...
                      <td style={td}>
                        <span
                          style={{
                            fontWeight: results > 0 ? 700 : 400,
                            color: results > 0 ? '#6366f1' : '#cbd5e1',
                          }}
                        >
