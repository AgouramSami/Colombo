  ...
  const [
    ...
    (() => {
      let q = db
        .from('race_pdfs')
        .select(
          'id, pdf_url, pdf_title, parse_status, parse_method, parsed_at, races(race_date, release_point, category)',
        )
        ...
        {[
          {
            ...
            value: (pagesCrawled ?? 0).toLocaleString('fr-FR'),
            color: '#0f172a',
          },
          { label: 'PDFs traités', value: pdfsSuccess ?? 0, color: '#16a34a' },
          {
            label: 'PDFs échoués',
            value: pdfsFailed ?? 0,
            color: (pdfsFailed ?? 0) > 0 ? '#dc2626' : '#94a3b8',
          },
          {
            label: 'Taux succès',
            value: `${successRate}%`,
          ...
          {
            label: 'Courses en DB',
            value: (totalRaces ?? 0).toLocaleString('fr-FR'),
            color: '#6366f1',
          },
          {
            label: 'Pigeons en DB',
            value: (totalPigeons ?? 0).toLocaleString('fr-FR'),
            color: '#0f172a',
          },
        ].map((k) => (
          <div
      ...
      >
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10 }}>
          ...
          <span style={{ fontSize: 14, color: '#64748b' }}>
            {pdfsSuccess ?? 0} / {totalPdfs ?? 0} PDFs
          </span>
        </div>
        ...
        <div style={{ marginTop: 8, fontSize: 12, color: '#94a3b8' }}>
          {successRate}% de succès · {pdfsFailed ?? 0} échec{(pdfsFailed ?? 0) > 1 ? 's' : ''} en
          quarantaine
        </div>
      </div>
      ...
      >
        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 600 }}>
            ...
            <tbody>
              {(recentPdfs ?? []).map((p) => {
                const race = p.races as unknown as {
                  race_date: string;
                ...
                return (
                  <tr key={p.id} className="admin-tr">
                    ...
                    <td style={{ ...td, color: '#94a3b8', fontSize: 12, whiteSpace: 'nowrap' }}>
                      {p.parsed_at
                        ? new Date(p.parsed_at).toLocaleDateString('fr-FR', {
                            day: '2-digit',
                            month: 'short',
                            hour: '2-digit',
                            minute: '2-digit',
