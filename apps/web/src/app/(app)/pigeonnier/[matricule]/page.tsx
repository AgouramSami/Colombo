...
export default async function PigeonDetailPage({
  ...
  const career: CareerEntry[] = (results ?? []).map((r) => {
    const race = r.races as unknown as { race_date: string; release_point: string; category: string; distance_min_km: number | null; distance_max_km: number | null } | null;
    const velocity = parseFloat(r.velocity_m_per_min ?? '0');
    const pct = r.n_engagement && r.n_engagement > 0 ? Math.round((r.place / r.n_engagement) * 100) : null;
    return {
      date: race?.race_date ?? '',
    ...
    <div style={{ minHeight: '100vh', background: 'var(--cb-bg)' }}>
      ...
      <header className="cb-topbar">
        ...
        <nav className="cb-topbar__nav">
          <a href="/pigeonnier" data-current="false">Pigeonnier</a>
        </nav>
      ...
      <main style={{ maxWidth: 1200, margin: '0 auto', padding: '24px clamp(16px, 4vw, 40px) 80px' }}>
        <Link href="/pigeonnier" className="cb-btn cb-btn--ghost" style={{ minHeight: 44, padding: '0 14px', marginBottom: 18, display: 'inline-flex' }}>
        ...
        <div className="cb-card" style={{ overflow: 'hidden', marginBottom: 24 }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'minmax(260px, 320px) 1fr' }} className="cb-hero-grid">
            {/* Photo placeholder */}
            <div style={{ background: 'var(--cb-bg-sunken)', minHeight: 280, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <span className="cb-muted" style={{ fontSize: 14 }}>Photo à venir</span>
            </div>
            ...
            <div style={{ padding: '28px 32px' }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, marginBottom: 12, flexWrap: 'wrap' }}>
                <span className="cb-badge" style={{
                  background: pigeon.is_female ? 'color-mix(in oklab, #c2185b 12%, var(--cb-bg-sunken))' : 'var(--cb-accent-soft)',
                ...
                {bestPlace === 1 && (
                  <span className="cb-badge cb-badge--gold">
                    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M18 2H6v7a6 6 0 0 0 12 0V2z"/><path d="M4 22h16"/></svg>
                    Champion
                  </span>
                )}
              </div>

              <h1 className="cb-display" style={{ fontSize: 'clamp(2rem, 4vw, 3rem)', margin: '4px 0 6px' }}>
                {pigeon.name ?? <span className="cb-muted" style={{ fontStyle: 'italic', fontWeight: 400 }}>Sans nom</span>}
              </h1>
              <div className="cb-matricule cb-muted" style={{ fontSize: '1.0625rem', marginBottom: 22 }}>
                {displayMatricule}
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: 12, marginBottom: 20 }}>
                ...
                {avgVelocity && <MiniKPI label="Vitesse moy." value={avgVelocity.toFixed(0)} suffix="m/min" />}
...
function MiniKPI({ label, value, suffix, accent, small }: {
  label: string; value: string | number; suffix?: string; accent?: boolean; small?: boolean;
}) {
  return (
    <div style={{ padding: '10px 0' }}>
      <div className="cb-muted" style={{ fontSize: 12, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '.06em' }}>{label}</div>
      <div className="cb-display cb-tabular" style={{
        fontSize: small ? '1.125rem' : '1.625rem',
        color: accent ? 'var(--cb-accent)' : 'var(--cb-ink)',
        marginTop: 2,
      }}>
        {value}
        {suffix && <span className="cb-muted" style={{ fontSize: 11, fontWeight: 500, marginLeft: 4 }}>{suffix}</span>}
      </div>
    </div>
  );
}
